...
impl CgroupReader {
    ...
    pub(crate) fn processes(&self, cgroup: &Path) -> Result<HashSet<u32>> {
        ...
        validate_cgroup_type(&cgroup_type)
            .with_context(|| format!("cgroup {} is not an exact domain", cgroup.display()))?;
...
fn validate_cgroup_type(value: &str) -> Result<()> {
    if value.trim_end() != "domain" {
        bail!("unsupported cgroup type {:?}", value.trim_end());
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn accepts_only_normal_domain_cgroups() {
        assert!(validate_cgroup_type("domain\n").is_ok());
        for value in ["threaded\n", "domain threaded\n", "domain invalid\n"] {
            assert!(validate_cgroup_type(value).is_err());
        }
    }
}
