...
impl<'a> Scheduler<'a> {
    ...
    fn sync_gpu_pids(&mut self) -> Result<()> {
        ...
        let count = match nvml.device_count() {
            Ok(count) => count,
            Err(error) => {
                debug!("NVML device count failed; clearing GPU workload hints: {error}");
        ...
        for i in 0..count {
            let node = match gpu_index_to_node.get(&i) {
                Some(&n) => n,
                None => {
                    debug!("NVML device {i} has no NUMA mapping; not expanding GPU cgroups");
                    snapshot_complete = false;
                    continue;
                }
            };
            let device = match nvml.device_by_index(i) {
                ...
                Err(error) => {
                    debug!(
                        "NVML device {i} could not be queried; not expanding GPU cgroups: {error}"
                    );
                    snapshot_complete = false;
                    continue;
                }
            ...
            if threshold > 0 {
                ...
                match device.process_utilization_stats(None::<u64>) {
                    ...
                    Err(_) => {
                        ...
                        {
                            debug!("NVML device {i} process snapshot is incomplete: {error:#}");
                            snapshot_complete = false;
                        }
                    }
                }
        ...
        let max_entries = map.max_entries() as usize;
